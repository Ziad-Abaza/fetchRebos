...
function searchGithubRepos(searchTerm) {
    ...
    fetch(`https://api.github.com/search/repositories?q=${searchTerm}+language:english&sort=updated&order=desc&per_page=100`)
        .then(response => response.json())
        .then(data => {
...
function createRepoCard(username, repo) {
    ...
    repoDescription.textContent = `Description: ${repo.description || 'No description'}`;
